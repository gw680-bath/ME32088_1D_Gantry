...
# Target structure: dictionary mapping target names to their ID numbers
TARGET_IDS = {
    "Target_1": 1,
    "Target_2": 2,
    "Target_3": 3,
...
# Global variable to store the result when used as a module
_result = None
...
def on_submit():
    ...
    # Store result and close window if being used as module
    _result = {
        'mode': choice,
        'targets': target_list,
        'target_ids': [TARGET_IDS.get(t, None) for t in target_list if t.startswith('Target_')],
        'manual_value': manual_var.get() if choice == "Manual Mode" else None
    }
...
def run_gui():
    """Run the GUI and return the selected targets configuration.
    Returns a dictionary with 'mode', 'targets', 'target_ids', and 'manual_value'.
    """
    global _result, root, controls_frame, image_frame, selected_option, manual_var
    global manual_frame, targets_frame, target_entries, target_values
    global targets_list_frame, image_canvas, target_image_ref
